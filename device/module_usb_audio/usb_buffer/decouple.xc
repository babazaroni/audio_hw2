...
#define MAX_DEVICE_AUD_PACKET_SIZE_MULT_HS ((MAX_FREQ/8000+1)*4)  // tried doubling here but got memory exception
#else
...
#ifdef KMI
#define MAX_DEVICE_AUD_PACKET_SIZE_HS ((MAX_FREQ/8000+1)*NUM_USB_CHAN_IN*4)  // tried doubling here but got distorted audio
#else
...
void handle_audio_request(chanend c_mix_out)
{
    ...
    static unsigned int dac_samples[2][32], adc_samples[2][32];
    static unsigned int buffer_select = 0;
    unsigned int dac_buffer_address, adc_buffer_address;

    inuint ( c_mix_out );


    asm("mov %0, %1":"=r"(dac_buffer_address):"r"(dac_samples[buffer_select]));
    asm("mov %0, %1":"=r"(adc_buffer_address):"r"(adc_samples[buffer_select]));
    outuint( c_mix_out, dac_buffer_address );

#ifdef XSCOPE_DEBUG_POWER // 17 handle_audio_request
    if (debug_count_2)
    {
        xscope_int(XSCOPE_POWER,17);
        debug_count_2--;
    }
#endif

    outuint( c_mix_out, adc_buffer_address );
...
__builtin_unreachable();
                ...
                for(int i = 0; i < g_numUsbChan_In; i++)
                {
                    /* Receive sample */
#ifdef KMI
                    int sample = bitrev(adc_samples[buffer_select][i]);
#else
...
#pragma xta endpoint "mixer_request"
                    ...
                    if (i<8)
                        dac_samples[buffer_select][i] = bitrev(sample);
                    else
                    {
#endif
                        ...
                        h <<= 3;
#ifdef KMI
                        dac_samples[buffer_select][i] = bitrev(h);
#else
                        outuint(c_mix_out, h);
#endif //KMI
#else
...
//            debug_buffer[debug_index++] = totalSampsToWrite;   // 24 for 96000
...
#else
//PACKET_SIZE_BUG_96KHZ
//            debug_buffer[debug_index++] = totalSampsToWrite;
//            debug_buffer[debug_index++] = g_curSubSlot_In;
//            debug_buffer[debug_index++] = g_numUsbChan_In;
//            debug_buffer[debug_index++] = g_maxPacketSize;
...
)
{
     ...
     * Note: assumed that buffer_c_audioControl is also setup before these globals are !0 */
#if (NUM_USB_CHAN_OUT > 0)
    ...
    while(1)
    {
...

        {
            ...
            if (tmp)
            {
                ...
                outuint(c_mix_out, tmp);

#ifdef XSCOPE_DEBUG_FEEDBACK
                            xscope_int(XSCOPE_FEEDBACK_SR,0);
                            xscope_int(XSCOPE_FEEDBACK_SR,0);
                            xscope_int(XSCOPE_FEEDBACK_SR,tmp);
                            xscope_int(XSCOPE_FEEDBACK_SR,0);
                            xscope_int(XSCOPE_FEEDBACK_SR,0);
#endif
                ...
                SET_SHARED_GLOBAL(g_powerChange_flag, 0);
            ...
            if (tmp == SET_SAMPLE_FREQ)
            {
                ...
                if (sampFreq==AUDIO_START_FROM_DFU)
                    show_debug();
#endif
...
#ifdef XSCOPE_DEBUG_POWER // 4
                xscope_int(XSCOPE_POWER,4);
#endif
...
#ifdef XSCOPE_DEBUG_POWER // 5
                xscope_int(XSCOPE_POWER,5);
#endif
