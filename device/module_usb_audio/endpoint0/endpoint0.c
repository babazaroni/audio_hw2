...
/* Support for C */
#define null 0
#define inuint(c, x)    asm ("in %1,res[%0]" ::  "r" (x) ,"r" (c) )
...
extern int buffer_c_audioControl;



void handle_meta_request(chanend c_meta_ctl)
{
    unsigned sample_rate;

    inuint ( c_meta_ctl,sample_rate );

//    asm("out res[%0],%1"::"r"(buffer_c_audioControl),"r"(SET_SAMPLE_FREQ));
//    asm("out res[%0],%1"::"r"(buffer_c_audioControl),"r"(sample_rate));
//    asm("chkct res[%0],%1"::"r"(buffer_c_audioControl),"r"(XS1_CT_END));


    outuint(buffer_c_audioControl, SET_SAMPLE_FREQ);
    outuint(buffer_c_audioControl, sample_rate);

    /* Wait for handshake back - i.e. PLL locked and clocks okay */
#ifdef XSCOPE_DEBUG_FEEDBACK
#endif
    chkct(buffer_c_audioControl, XS1_CT_END);
...
//        configure ports for spi, looks like flash code configures the ports
        // No Handshake
        DFU_mode_active = 1;  // qqq
        dfu_debug_put_c(30);
    }
#endif
    dfu_debug_put_c(31);
...
#ifdef KMI
//    set_interrupt_handler(handle_meta_request, 200, 1, c_meta_ctl, 0);
#endif
...
#ifdef XSCOPE_DEBUG_FEEDBACK
        xscope_int(XSCOPE_ENDPOINT,result);
#endif
                ...
                case USB_BMREQ_D2H_CLASS_INT:
                    {
                        ...
                        if (interfaceNum == DFU_IF)
                        {
                            ...
                            if ((DFU_IF == INTERFACE_NUMBER_DFU) &&
                                ...
                                (sp.bRequest != XMOS_DFU_READ_FLASH_ENTRY) &&
                                (sp.bRequest != XMOS_DFU_READ_FLASH_FORMAT))
                            {
                                // Stop audio
                                outuint(c_audioControl, SET_SAMPLE_FREQ);
                                outuint(c_audioControl, AUDIO_STOP_FOR_DFU);
                            ...
                            dfu_debug_msg(0,g_interfaceAlt[sp.wIndex]);
#ifdef __XC__
...
//                dfu_debug_toggle(chan_dfu_debug,3);
